...
function getProfilo() {
	return "${user.currentProfilo.codice}";
}
...
function getDataDaControllo(controllo) {
	if (!controllo.selector) controllo = $(controllo);
	if (controllo.size() < 1) return null;
	var d = controllo.val();

	if (d.length >= 6 && _isValidDate(d)) {
		var dsplit = d.split("/");
		var result = Date.parse(dsplit[1] + "/" + dsplit[0] + "/" + dsplit[2]);
		result.originalText = d;
		return result;
	}
...
function setLoading(control) {
	if (!control.selector) control = $(control);
	control.attr("disabled", "disabled");
	control.append("<span class='inline-button-loading'> <img src='${contextPath}/img/loading.gif' style='width:16px;display:inline;' /></span>");
}

function clearLoading(control) {
	if (!control.selector) control = $(control);
	control.removeAttr("disabled");
	control.find(".inline-button-loading").remove();
}
...
var VALIDATOR_VALID_URL = "valid-url";
...
var customValidators = {
	...
	"valid-url" : function(value) {
		if (value === null || value === undefined || value === "") return true;
		if (!_isValidUrl(value)) return "inserire un URL valido";
		return true;
	}
};
...
var CookieManager = {
	...
	get : function(name) {
	    ...
	    for (var i = 0; i < ca.length; i++) {
	        ...
	        if (c.indexOf(nameEQ) === 0) {
		        var decodedData = JSON.parse(decodeURIComponent(c.substring(nameEQ.length, c.length)));
		        return CookieManager._build(decodedData, name);
	        }
	    }
	    return CookieManager._build({}, name);
	},
	...
	remove : function(name) {
		CookieManager._del(name);
	},
	
	_store : function(name, value, days) {
		var expires;
	...
	_del : function(name) {
		CookieManager._store(name, "", -1);
	},

	_build : function(data, name) {
		data['___name'] = name;
		...
		data.save = function() {
			...
			$.each(data, function(k,v) {
				...
				if (k.startsWith("___")) return;
				cloned[k] = v;
			});
			CookieManager._store(data['___name'], cloned);
		};

		data.remove = function() {
			CookieManager._del(data['___name']);
		}
